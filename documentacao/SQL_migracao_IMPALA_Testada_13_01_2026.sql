...
SELECT 
    ...
    COALESCE(tx.co_protocolo_sei, '') AS co_protocolo_sei,
    ...
    COALESCE(
        CONCAT_WS('||', COLLECT_SET(CAST(tx.id_arquivo_anexo AS STRING))),
        ''
		
    ) AS special_attachments,
    COALESCE(
        CONCAT_WS('||', COLLECT_SET(CAST(tx.ds_arquivo_gravado AS STRING))),
        ''
