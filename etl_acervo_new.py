...
-- VERSÃO PARA IMPALA (usando GROUP_CONCAT DISTINCT)
-- =====================================================================
-- Adaptações para Impala:
-- - GROUP_CONCAT com DISTINCT para evitar duplicatas e separador '||'
...
SELECT 
    ...
    CASE 
        WHEN CAST(ta.ic_principal AS STRING) IN ('1', 'true') THEN 'Sim'
        WHEN CAST(ta.ic_principal AS STRING) IN ('0', 'false') THEN 'Não'
        ELSE ''
    END AS ic_principal,
    ...
    -- Campo 7: id_arquivo_acervo_1 (Arquivos associados) - IMPALA: GROUP_CONCAT DISTINCT
    COALESCE(GROUP_CONCAT(DISTINCT CAST(tq.id_arquivo_acervo_1 AS STRING), '||'), '') AS id_arquivo_acervo_1,
    
    -- Campo 8: id_arquivo_acervo_2 (Arquivos associados) - IMPALA: GROUP_CONCAT DISTINCT
    ...
    CASE 
        WHEN CAST(td.ic_vinculado_spu AS STRING) IN ('1', 'true') THEN 'Sim'
        WHEN CAST(td.ic_vinculado_spu AS STRING) IN ('0', 'false') THEN 'Não'
        ELSE ''
    ...
    -- Campo 24: ds_autor_copia - IMPALA: GROUP_CONCAT DISTINCT
    COALESCE(GROUP_CONCAT(DISTINCT tac.ds_autor_copia, '||'), '') AS ds_autor_copia,
    
    -- Campo 25: ds_autor_original - IMPALA: GROUP_CONCAT DISTINCT
    COALESCE(GROUP_CONCAT(DISTINCT tao.ds_autor_original, '||'), '') AS ds_autor_original,
    
    -- Campo 26: ds_processo_vinculado - IMPALA: GROUP_CONCAT DISTINCT
    COALESCE(GROUP_CONCAT(DISTINCT tpv.ds_processo_vinculado, '||'), '') AS ds_processo_vinculado,
    
    -- Campo 27: ds_palavra_chave_iconografico - IMPALA: GROUP_CONCAT DISTINCT
    ...
    CASE 
        WHEN CAST(tdc.ic_iconografia AS STRING) IN ('1', 'true') THEN 'Sim'
        WHEN CAST(tdc.ic_iconografia AS STRING) IN ('0', 'false') THEN 'Não'
        ELSE ''
    ...
    CASE 
        WHEN CAST(tdc.ic_notacao_kardex AS STRING) IN ('1', 'true') THEN 'Sim'
        WHEN CAST(tdc.ic_notacao_kardex AS STRING) IN ('0', 'false') THEN 'Não'
        ELSE ''
    END AS ic_notacao_kardex,
    
    -- Campo 37: ic_especie_cartografia (FLAG) - converter para Sim/Não
    CASE 
        WHEN CAST(tdc.ic_especie_cartografia AS STRING) IN ('1', 'true') THEN 'Sim'
        WHEN CAST(tdc.ic_especie_cartografia AS STRING) IN ('0', 'false') THEN 'Não'
        ELSE ''
    END AS ic_especie_cartografia,
    
    -- Campo 38: ic_especie_fotografia (FLAG) - converter para Sim/Não
    CASE 
        WHEN CAST(tdc.ic_especie_fotografia AS STRING) IN ('1', 'true') THEN 'Sim'
        WHEN CAST(tdc.ic_especie_fotografia AS STRING) IN ('0', 'false') THEN 'Não'
        ELSE ''
    ...
    -- Campo 74: ds_interessado - IMPALA: GROUP_CONCAT DISTINCT
    COALESCE(GROUP_CONCAT(DISTINCT ti.ds_interessado, '||'), '') AS ds_interessado,
    
    -- Campo 76: ds_processo_antecedente - IMPALA: GROUP_CONCAT DISTINCT
...
WHERE 
    CAST(ta.ic_publicado_internet AS STRING) IN ('1', 'true')
